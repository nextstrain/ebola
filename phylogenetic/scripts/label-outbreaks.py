        ...
        if outbreak_nextclade:=cas.get(start_node, False):
            ...
            for node in start_node.find_clades():
                if node.name in nodes:
                    print("...overwriting", nodes[node.name]['outbreaknextclade'], 'with', outbreak_nextclade)
                nodes[node.name] = {
                    'outbreaknextclade': outbreak_nextclade,
                    'outbreakgeo': outbreak_geo
                }

            # label CA branch with a branch label
            branches[start_node.name] = {'labels': {'outbreaknextclade': outbreak_nextclade}}
            if geo_names[outbreak_nextclade]['label']:
                branches[start_node.name]['labels']['outbreakgeo'] = outbreak_geo
